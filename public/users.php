    ...
    <div style="display: flex; justify-content: start; align-items: center; padding: 1em 2em;">
      ...
      <div style="margin-left: 2em;">
        <?php if(!empty($_SESSION['login_user_id']) && $user['id'] === $_SESSION['login_user_id']): ?>
          これはあなたです!
        <?php elseif(!empty($_SESSION['login_user_id']) && in_array($user['id'], $followee_user_ids)): ?>
          フォロー済
          (
            <a href="./follow_remove.php?followee_user_id=<?= $user['id'] ?>">
              フォロー解除
            </a>
          )
        <?php elseif(!empty($_SESSION['login_user_id'])): ?>
          <a href="./follow.php?followee_user_id=<?= $user['id'] ?>">フォローする</a>
        <?php else: ?>
          フォローするには<a href="/login.php">ログイン</a>が必要です
        <?php endif; ?>
      </div>
    </div>
